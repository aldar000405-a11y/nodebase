...
export const httpRequestExecutor: NodeExecutor<HttpRequestData> = async ({
    ...
    const configHash = Buffer.from(JSON.stringify(data)).toString('hex').slice(0, 8);
    ...
    try {
        console.log(`[Executor ${nodeId.slice(-8)}] Entering step.run with id: ${stepId}`);
        ...
        const result = await step.run(stepId, async () => {
            ...
            if (["POST", "PUT", "PATCH"].includes(method)) {
                ...
                try {
                    ...
                    throw new NonRetriableError(`HTTP Request node: Invalid JSON body - ${resolved.substring(0, 100)}`);
                }
